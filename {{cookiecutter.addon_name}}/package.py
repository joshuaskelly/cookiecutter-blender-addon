...
def run():
    ...
    zip_entries = gather_files('{{cookiecutter.module_name}}')

    for module in get_required_modules():
        module = importlib.import_module(module)
        zip_entries += gather_files(os.path.dirname(module.__file__), os.path.join('{{cookiecutter.module_name}}', 'modules'))

    filename = f'{{cookiecutter.module_name}}-{% raw %}{{% endraw %}{{cookiecutter.module_name}}.__version__{% raw %}}{% endraw %}.zip'
